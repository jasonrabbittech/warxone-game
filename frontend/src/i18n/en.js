export default {
    mainMenu: { title:"WARXONE", tagline:"Conquer Earth & Mars", newGame:"New Game", loadGame:"Load Game", story:"The Story", settings:"Settings", howToPlay:"How to Play", login:"Login" },
    auth: { loginTitle:"Login", email:"Email", password:"Password", loginBtn:"Login", noAccount:"Don't have an account?", signupLink:"Sign Up", hasAccount:"Already have an account?", loginLink:"Login", logout:"Logout", orContinueWith:"or continue with", googleBtn:"Sign in with Google" },
    signup: { title:"Sign Up", email:"Email", password:"Password (min 8)", confirm:"Confirm Password", btn:"Sign Up", back:"Back to Menu" },
    setup: { title:"Setup Your Country", countryName:"Country Name", capitalName:"Capital Name", flag:"Flag", startingCountry:"Starting Country", startBtn:"Start Game", backBtn:"Back to Menu" },
    map: { tokens:"Tokens", pop:"Pop", lvl:"Lvl", land:"Land", conns:"Conns", earthPercent:"\u{1F30D}%", quiz:"Quiz", collect:"Collect", pause:"Pause", mapNav:"Map" },
    tutorial: { title:"How to Play", text:"1. Sign up for 10 tokens.\n2. Choose a country and conquer.\n3. Attack adjacent countries (yellow).\n4. Win battles for tokens & chests (\u{1F9E7}).\n5. Collect cards from the Shop.\n6. Establish Connections for daily tokens.\n7. Join alliances.\n8. Take the History Quiz.\n9. Conquer 90% of Earth to unlock Mars!" },
    battle: { title:"BATTLEFIELD", attacker:"You", defender:"Enemy", auto:"Auto", retreat:"Retreat", close:"Close" },
    card: { title:"Cards", basic:"Basic Pack", cost:"5 Tokens", redeem:"Redeem", yourCards:"Your Cards", shopNav:"Shop" },
    connection: { title:"Connections" },
    alliance: { title:"Alliance" },
    quiz: { 
      title:"History Quiz",
      selectDifficulty:"Select Difficulty",
      easy:"Easy",
      medium:"Medium",
      hard:"Hard",
      superHard:"Super Hard",
      invincibleHard:"Invincible Hard",
      locked:"Locked (Level {level} required)",
      dailyLimit:"Daily Limit Reached",
      dailyLimitMsg:"You have already completed a quiz today. Come back tomorrow!",
      nextAttempt:"Next attempt in: {time}",
      question:"Question {current}/{total}",
      submit:"Submit",
      next:"Next",
      viewResults:"View Results",
      correct:"Correct!",
      incorrect:"Incorrect",
      timeout:"Time is up!",
      quizComplete:"Quiz Complete!",
      score:"Score: {correct}/{total}",
      tokensEarned:"Tokens: {tokens}",
      playAgain:"Play Again",
      backToMenu:"Back to Menu",
    },
    settings: { title:"Settings", save:"Save", reset:"Reset", export:"Export", import:"Import", pause:"Paused", resume:"Resume", quit:"Quit to Menu" }
};
